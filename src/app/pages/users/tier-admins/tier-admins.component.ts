...
@Component({
    selector: 'app-tier-admins',
    templateUrl: './tier-admins.component.html',
    styleUrls: ['./tier-admins.component.scss']
})
export class TierAdminsComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.isLoading = true;
        this.usersService.usersControllerGetUsersPage(1, 100, 'TIER_ADMIN').subscribe((users: User[]) => {
            console.log('Tier Admin ---', users);
    ...
    gotoTierAdmin(element: User) {
        const id = element.id;
        this.router.navigate(['tier-admin-detail'], { queryParams: { id: id }, relativeTo: this.route, state: element });
    }
